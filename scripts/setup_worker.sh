...
MANAGER_IP=""
SWARM_TOKEN=""
HOSTNAME_PREFIX="worker"           # workers serão: worker-01, worker-02 ...
NFS_SERVER=""                      # IP do servidor NFS (deixe vazio para pular)
NFS_MOUNT_PATH="/mnt/cluster"      # onde montar o volume compartilhado
...
# -----------------------------------------------------------------------------
section "1/7 — Definindo hostname"

MAC=$(cat /sys/class/net/$(ip route show default | awk '/default/ {print $5}' | head -1)/address \
      | tr -d ':' | tail -c 5)
...
log "Hostname definido: $NEW_HOSTNAME"

# -----------------------------------------------------------------------------
# 2. ATUALIZAÇÃO DO SISTEMA
# -----------------------------------------------------------------------------
section "2/7 — Atualizando sistema"
...
# 3. DEPENDÊNCIAS BASE
# -----------------------------------------------------------------------------
section "3/7 — Instalando dependências"
...
# -----------------------------------------------------------------------------
section "5/7 — Aplicando otimizações"

# Desabilitar swap (recomendado para Docker Swarm)
swapoff -a
...
# -----------------------------------------------------------------------------
section "6/7 — Configurando NFS"

if [[ -n "$NFS_SERVER" ]]; then
    mkdir -p "$NFS_MOUNT_PATH"

    # Testa conectividade antes de montar
    if ping -c 1 -W 2 "$NFS_SERVER" &>/dev/null; then
        ...
        mount -a 2>/dev/null && log "NFS montado em $NFS_MOUNT_PATH" \
...
# 7. ENTRAR NO SWARM
# -----------------------------------------------------------------------------
section "7/7 — Entrando no Docker Swarm"

# Verifica se já é membro
if docker info 2>/dev/null | grep -q "Swarm: active"; then
    warn "Este nó já faz parte de um Swarm. Pulando."
else
    # Testa conectividade com o manager
    if ! curl -sf --connect-timeout 5 "http://${MANAGER_IP}:2377" &>/dev/null; then
        warn "Manager em $MANAGER_IP pode estar inacessível na porta 2377"
    fi
